...
#define DARK_CHAT_BACKGROUND_COLOR                                             \
  lv_color_hex(0x1E1E1E) // Slightly lighter than background
...
#define LIGHT_CHAT_BACKGROUND_COLOR                                            \
  lv_color_hex(0xE0E0E0)                               // Light gray background
...
RgbLcdDisplay::RgbLcdDisplay(esp_lcd_panel_io_handle_t panel_io,
  ...
  const lvgl_port_display_cfg_t display_cfg = {
      ...
      .buffer_size = static_cast<uint32_t>(width_ * 10),
      .double_buffer = true,
      .trans_size = static_cast<uint32_t>(width_ * 40),
      .hres = static_cast<uint32_t>(width_),
      .vres = static_cast<uint32_t>(height_),
      .monochrome = false,
      .rotation =
...
void LcdDisplay::SetupUI() {
  ...
  lv_obj_set_style_bg_color(content_, LIGHT_BACKGROUND_COLOR, 0);
  ...
  lv_obj_set_width(chat_message_label_,
                   LV_HOR_RES * 0.9); // 限制宽度为屏幕宽度的 90%
  lv_label_set_long_mode(chat_message_label_,
                         LV_LABEL_LONG_WRAP); // 设置为自动换行模式
  ...
  lv_obj_set_style_pad_left(network_label_, 0, 0);
